...
void Tracking::Track()
{
    ...
    if(mState==NOT_INITIALIZED)
    {
        ...
        std::cout<<"Inside mstate not initialized"<<std::endl;
        if(mSensor==System::STEREO || mSensor==System::RGBD || mSensor==System::IMU_STEREO)
    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            std::cout<<"Inside md only tracking"<<endl;
            // State OK
            ...
            {
                std::cout<<"Inside recently lost"<<endl;
                if (mState == RECENTLY_LOST)
                {
                    ...
                    {   
                        std::cout<<"Inside relocaliztion"<<endl;
                ...
                else if (mState == LOST)
                {
                    ...
                    std::cout<<"Inside state is lost"<<std::endl;
        ...
        {   
            ...
            std::cout<<"Inside localization mode -- local mapping deactivated"<<endl;
        ...
        {   
            std::cout<<"Inside mstate is ok"<<endl;
            if (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO)
            {
        ...
        {
            std::cout<<"Inside motion model"<<endl;
            // Update motion model
        ...
        if(mState==LOST)
        {   
            cout<<"Inside mstate is lost"<<endl;
    ...
    if(mState==OK || mState==RECENTLY_LOST)
    {
        ...
        std::cout<<"Inside mstate is recently lost"<<endl;
        if(!mCurrentFrame.mTcw.empty())
        {
        ...
        {
            std::cout<<"Inside this can happen if tracking is lost"<<endl;
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    // Create KeyFrames
    //mpAtlas->segment->find_or_construct<KeyFrame>
    ...
    for(size_t i=0; i<mvIniMatches.size();i++)
    {
        ...
        boost::interprocess::offset_ptr<MapPoint>  pMP = mpAtlas->segment->find_or_construct<MapPoint>(boost::interprocess::anonymous_instance)(worldPos,pKFcur,mpAtlas->GetCurrentMap());
...
void Tracking::UpdateLastFrame()
{
    ...
    for(size_t j=0; j<vDepthIdx.size();j++)
    {
        ...
        if(bCreateNew)
        {
            ...
            //mpAtlas->segment->find_or_construct<MapPoint>(boost::interprocess::anonymous_instance)
            //boost::interprocess::offset_ptr<MapPoint>  pNewMP = new MapPoint(x3D,mpAtlas->GetCurrentMap(),&mLastFrame,i);
            boost::interprocess::offset_ptr<MapPoint>  pNewMP = mpAtlas->segment->find_or_construct<MapPoint>(boost::interprocess::anonymous_instance)(x3D,mpAtlas->GetCurrentMap(),&mLastFrame,i);
        ...
        if(!vDepthIdx.empty())
        {
            ...
            for(size_t j=0; j<vDepthIdx.size();j++)
            {
                ...
                if(bCreateNew)
                {
                    ...
                    boost::interprocess::offset_ptr<MapPoint>  pNewMP = mpAtlas->segment->find_or_construct<MapPoint>(boost::interprocess::anonymous_instance)(x3D,pKF,mpAtlas->GetCurrentMap());
...
void Tracking::CreateNewMapPoints()
{
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        ...
        for(int ikp=0; ikp<nmatches; ikp++)
        {
            ...
            if(bStereo1)
                cosParallaxStereo1 = cos(2*atan2(mpLastKeyFrame->mb/2,mpLastKeyFrame->mvDepth->at(idx1)));//cosParallaxStereo1 = cos(2*atan2(mpLastKeyFrame->mb/2,mpLastKeyFrame->mvDepth[idx1]));
            else if(bStereo2)
                cosParallaxStereo2 = cos(2*atan2(pKF2->mb/2,pKF2->mvDepth->at(idx2)));//cosParallaxStereo2 = cos(2*atan2(pKF2->mb/2,pKF2->mvDepth[idx2]));
            ...
            boost::interprocess::offset_ptr<MapPoint>  pMP = mpAtlas->segment->find_or_construct<MapPoint>(boost::interprocess::anonymous_instance)(x3D,mpLastKeyFrame,mpAtlas->GetCurrentMap());
