...
cv::Mat Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename)
{
    ...
    cout<<"ORB Extraction: "<< mCurrentFrame.mTimeORB_Ext <<endl;
    ...
    else
    {
        ...
        if(bOK || mState==RECENTLY_LOST)
        {
            ...
            if(bNeedKF && (bOK|| (mState==RECENTLY_LOST && (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO)))){
                ...
                new_keyframe+=1;
                //Aditya: Edit
                std::cout<<"*****---- Total Frames checked: "<<total_frames<<"  New Keyframes: "<<new_keyframe<<std::endl;
            }
...
#ifdef CUDA_ENABLED
__global__ void print_hello(cv::Mat matrix){
	float * data =(float *)( (matrix.data));
	printf("Hello from GPU ID:, first matrixdata is  %f\n",data[0]);
}
...
*/
__global__ void isInFrustumCUDA(Frame *thisFrame, void **pMPe, int num_points, float viewingCosLimit,  float *parameters,  bool *ans, float* mRcwd, float* mtcwd, float*mOwd, float mnMinX, float mnMaxX, float mnMinY, float mnMaxY, float mbf, int Nleft,float logscalefactor, int mnScaleLevels);
...
__global__ void isInFrustumCUDA(Frame *thisFrame, boost::interprocess::offset_ptr<MapPoint> *pMPe, int num_points, float viewingCosLimit,  float *parameters,  bool *ans, float* mRcwd, float* mtcwd, float*mOwd, float mnMinX, float mnMaxX, float mnMinY, float mnMaxY, float mbf, int Nleft,float logscalefactor, int mnScaleLevels)
{
	...
	float * normal = (float *)(pMP->mNormalVector.data);
	//printf("Failed after this\n");
	//float mnMinX = thisFrame->mnMinX; 
	//float mnMaxX= thisFrame->mnMaxX;
	//float mnMinY = thisFrame->mnMinY;
	//float mnMaxY = thisFrame->mnMaxY; 
	//float mbf = thisFrame->mbf;
	//int Nleft = thisFrame->Nleft;
	...
	if( Nleft == -1){
	        ...
	        Pc[0] = P[0]*mRcwd[0]+P[1]*mRcwd[1]+P[2]*mRcwd[2]+mtcwd[0];
	        Pc[1] = P[0]*mRcwd[3]+P[1]*mRcwd[4]+P[2]*mRcwd[5]+mtcwd[1];
	        Pc[2] = P[0]*mRcwd[6]+P[1]*mRcwd[7]+P[2]*mRcwd[8]+mtcwd[2];
	        ...
	        float uv_x = parameters[0]*Pc[0]/Pc[3]+parameters[2];
	        float uv_y = parameters[1]*Pc[2]/Pc[3]+parameters[3];
	*/
	        //apparently projection is Kannalabrandt
	        float x2_plus_y2 = Pc[0] * Pc[0] + Pc[1] * Pc[1];
	        //if((idx%100)==0){
	        //	printf("idx: %d x2_plus_y2: %f\n",idx,x2_plus_y2);
	        //}
	        float theta = atan2f(sqrtf(x2_plus_y2), Pc[2]);
	        float psi = atan2f(Pc[1], Pc[0]);
	        ...
	        float r = theta + parameters[4] * theta3 + parameters[5] * theta5
	                                + parameters[6] * theta7 + parameters[7] * theta9;

	        float uv_x = parameters[0] * r * cosf(psi) + parameters[2];
	        float uv_y = parameters[1] * r * sinf(psi) + parameters[3];
	        // cout << "c";
	        ...
	        float maxDistance = pMP->mfMaxDistance*1.2f;
	        float minDistance = pMP->mfMinDistance*1.2f;
	        float PO[3];
	        PO[0] = P[0]-mOwd[0];
	        PO[1] = P[1]-mOwd[1];
	        PO[2] = P[2]-mOwd[2];
	        ...
	        float viewCos = (normal[0]*PO[0]+normal[1]*PO[1]+normal[2]*PO[2])/dist;

	        if(viewCos<viewingCosLimit)
	        	ans[idx]= false;

	        // Predict scale in the image
	        //const int nPredictedLevel = pMP->PredictScaleCuda(dist,thisframe);
	        //predict scale code
	    float ratio;
    	//{
        //std::unique_lock<mutex> lock(mMutexPos);
        ratio = pMP->mfMaxDistance/dist;
        
    	//}
		//int nPredictedLevel = 0;
    	int nScale = (int) (ceilf(logf(ratio)/logscalefactor));
    	//printf("nScale: %d, ratio: %f, logscalefactor: %f, mnScaleLevels: %d, pc_dist: %f, viewCos: %f \n",nScale,ratio,logscalefactor,mnScaleLevels,Pc_dist,viewCos);
    	if(nScale<0)
        	nScale = 0;
    	else if(nScale>=mnScaleLevels)
        	nScale = mnScaleLevels-1;
		//nPredictedLevel = nScale;
		// Data used by the tracking
		pMP->mbTrackInView = true;
		//pMP->mTrackProjX = uv_x;
		pMP->mTrackProjXR = uv_x - mbf*invz;
		
		pMP->mTrackDepth = Pc_dist;
		// cout << "h";
		
		//pMP->mTrackProjY = uv_y;
		pMP->mnTrackScaleLevel= nScale;
		pMP->mTrackViewCos = viewCos;
		
		// cout << "i";
		//printf("IDX: %d is true",)
		ans[idx]= true;
		}
	//printf("Finished\n");
	}
...
void Tracking::SearchLocalPoints()
{
    ...
    int num_blocks = num_mapPoints/64;
    int remaining_blocks =  num_mapPoints%64;
    if (remaining_blocks > 0)
    	num_blocks+=1;
    
   // dim3 blocks(num_blocks,1,1);
    //dim3 threads_per_block(64,1,1);
   // cudaHostRegister((void*)(mvpLocalMapPoints.at(0)->mNormalVector.data),sizeof(float)*8, cudaHostRegisterDefault);
   // print_hello<<<1,1>>>(mvpLocalMapPoints.at(0)->mNormalVector);
    //cudaDeviceSynchronize();

    

    //frame parameters
    mCurrentFrame.cam_parameter[0] = mCurrentFrame.mpCamera->getParameter(0);
    mCurrentFrame.cam_parameter[1] = mCurrentFrame.mpCamera->getParameter(1);
    mCurrentFrame.cam_parameter[2] = mCurrentFrame.mpCamera->getParameter(2);
    mCurrentFrame.cam_parameter[3] = mCurrentFrame.mpCamera->getParameter(3);
    mCurrentFrame.cam_parameter[4] = mCurrentFrame.mpCamera->getParameter(4);
    mCurrentFrame.cam_parameter[5] = mCurrentFrame.mpCamera->getParameter(5);
    mCurrentFrame.cam_parameter[6] = mCurrentFrame.mpCamera->getParameter(6);
    mCurrentFrame.cam_parameter[7] = mCurrentFrame.mpCamera->getParameter(7);
    
    mCurrentFrame.mrcw_f[0] = ((float*)(mCurrentFrame.mRcw.data))[0];
    mCurrentFrame.mrcw_f[1] = ((float*)(mCurrentFrame.mRcw.data))[1];
    mCurrentFrame.mrcw_f[2] = ((float*)(mCurrentFrame.mRcw.data))[2];
    mCurrentFrame.mrcw_f[3] = ((float*)(mCurrentFrame.mRcw.data))[3];
    mCurrentFrame.mrcw_f[4] = ((float*)(mCurrentFrame.mRcw.data))[4];
    mCurrentFrame.mrcw_f[5] = ((float*)(mCurrentFrame.mRcw.data))[5];
    mCurrentFrame.mrcw_f[6] = ((float*)(mCurrentFrame.mRcw.data))[6];
    mCurrentFrame.mrcw_f[7] = ((float*)(mCurrentFrame.mRcw.data))[7];
    mCurrentFrame.mrcw_f[8] = ((float*)(mCurrentFrame.mRcw.data))[8];
    //cout<<"CUrrentFrame 0"<<mCurrentFrame.mrcw_f[0];
    
    mCurrentFrame.mtcw_f[0] = ((float *)(mCurrentFrame.mTcw.data))[0];
    mCurrentFrame.mtcw_f[1] = ((float *)(mCurrentFrame.mTcw.data))[1];
    mCurrentFrame.mtcw_f[2] = ((float *)(mCurrentFrame.mTcw.data))[2];
    
    mCurrentFrame.mOw_f[0] = ((float *)(mCurrentFrame.mOw.data))[0];
    ...
    cudaHostRegister(mCurrentFrame.mOw_f, sizeof(float)*3,cudaHostRegisterDefault);
    cudaHostRegister(mCurrentFrame.answers,sizeof(bool)*10000,cudaHostRegisterDefault);
    //first few checks... in Frustum.
    memset(mCurrentFrame.answers,0,sizeof(bool)*10000);
    cout<<"Number of Mappoints: "<<num_mapPoints<<endl;
    isInFrustumCUDA<<<num_blocks,64>>>(&mCurrentFrame,mvpLocalMapPoints.data(),num_mapPoints,0.5,mCurrentFrame.cam_parameter,mCurrentFrame.answers,mCurrentFrame.mrcw_f,mCurrentFrame.mtcw_f,mCurrentFrame.mOw_f,mCurrentFrame.mnMinX,mCurrentFrame.mnMaxX,mCurrentFrame.mnMinY,mCurrentFrame.mnMaxY,mCurrentFrame.mbf,-1,mCurrentFrame.mfLogScaleFactor,mCurrentFrame.mnScaleLevels);
    cudaDeviceSynchronize();
    cudaHostUnregister(mvpLocalMapPoints.data());
	cudaHostUnregister(mCurrentFrame.cam_parameter);
	cudaHostUnregister(mCurrentFrame.mrcw_f);
	cudaHostUnregister(mCurrentFrame.mtcw_f);
	cudaHostUnregister(mCurrentFrame.mOw_f);
	cudaHostUnregister(mCurrentFrame.answers);
    ...
    */
    ...
    for(vector<boost::interprocess::offset_ptr<MapPoint> >::iterator vit=mvpLocalMapPoints.begin(), vend=mvpLocalMapPoints.end(); vit!=vend; vit++)
    {
        ...
        {
        	//cout<<"nscale: "<<pMP->mnTrackScaleLevel<<endl;
        	pMP->IncreaseVisible();
        	            nToMatch++;
        }
